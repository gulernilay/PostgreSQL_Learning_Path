...
--Tablonun tamamını değil, sadece verisini silmek istiyorsan DELETE ile farkını açıkla. (teorik)
--email_log adında var olduğunu düşündüğün tabloyu silmeden önce olup olmadığını kontrol et.
...
--ALTER :The PostgreSQL ALTER TABLE command is used to add, delete or modify columns in an existing table.You would also use ALTER TABLE command to add and drop various constraints on an existing table.
...
--------------------------------CREATE : Yeni veritabanı nesnesi oluşturur (tablo, view, procedure, function, index vs.)	
