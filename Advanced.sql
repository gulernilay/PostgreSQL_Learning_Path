...
--TRIGGERS- belirli bir veritabanı olayı gerçekleştiğinde otomatik olarak çalışan (callback) fonksiyonlardır.
...
--⏱️ BEFORE / AFTER / INSTEAD OF Seçimi
...
--🗑️ Tablo silinirse trigger da silinir
--Trigger’lar, ilişkilendirildikleri tablo silindiğinde otomatik olarak kaldırılır.


--🌐 Tablolar aynı veritabanında olmalı
--Trigger’ın değiştireceği tablo ile ilişkilendirildiği tablo/view aynı veritabanında bulunmalıdır.
--database.tablename kullanılamaz, sadece tablename yazılmalıdır.


--⚠️ CONSTRAINT trigger
...
--Trigger sadece tek bir tabloya atanır. 
--PostgreSQL’de trigger'lar tabloya özgü olarak tanımlanır. 
--Yani bir trigger, belirli bir tabloya veya view'e bağlıdır ve sadece o tablo üzerinde gerçekleşen belirli bir olay (INSERT, UPDATE, DELETE) 
...
--Örnek 2 : Kiralama yapılırken stok yetersizse engelle.rental eklemeden önce, film stoğu 0 ise işlemi engelle.
...
--ÖRNEK 3: Güncellenen filmin uzunluğu loglansın
--Eğer bir film güncellenirken length değeri değişirse, eski ve yeni değer loglansın.
...
--ÖRNEK 4: Soft Delete (Müşteri silinmesin, sadece işaretlensin)
--Gerçekten silmek yerine, active = false yap.
...
🎯 Ekstra: Hangi Trigger Türü Ne Zaman?
...
🚫 Neden 										Her Sütuna Ayrı Index Oluşturmak Kötüdür?
Sebep											Açıklama
...
⚠️ Query Planner karmaşıklaşır					PostgreSQL'in en uygun indexi seçmesi daha zorlaşır (plan cost yükselir).
...
🧠 Peki Alternatif Ne?
🔹 1. Birden fazla sütunu kapsayan bileşik index (composite index): 
...
⏱️ Ne zaman bekleme olur, ne zaman olmaz?
...
--⚙️ Kilitler otomatik mi verilir?
...
🔁 Kilit, işlem bitene kadar (commit/rollback) sürer.
❌ UNLOCK komutu yoktur.

⚠️ Deadlock (Kilit Çakışması) Nedir?
İki işlem birbirini bekliyorsa ve her biri diğerinin tuttuğu kilide ulaşmaya çalışıyorsa, deadlock oluşur.

PostgreSQL bu durumu otomatik tespit eder ve işlemlerden birini ROLLBACK yapar.
...
Örneğin: Bir tablonun satırında “şu anda biri çalışıyor” gibi bir bayrak yerine advisory lock kullanılır.
...
⚠️ Deadlock	İşlemler birbirini beklerse oluşur
...
--------öRNEK SORU :Her müşterinin yaptığı toplam ödeme tutarını gösteren bir sorgu yaz. ,

✅ Soru 1: Her müşterinin yaptığı toplam ödeme tutarını göster
🧠 Beklenen: Scalar Subquery kullanımını göstermek.
...
customer tablosundan her müşteri alınır.

Her satır için, payment tablosunda eşleşen toplam ödeme subquery ile hesaplanır.

✅ Soru 2: En çok kiralanan filmi getiren sorgu (tek satır)
🧠 Beklenen: ORDER BY + LIMIT 1 içeren subquery
...
rental → inventory üzerinden film_id’ye ulaşıyoruz.

En çok kiralanan film_id’yi alt sorguda buluyoruz.

Üst sorgu, film.title gibi detayları getiriyor.

✅ Soru 3: En az bir ödeme yapmış müşteriler
🧠 Beklenen: EXISTS veya IN kullanımı
...
EXISTS ifadesiyle sadece ödeme yapmış müşteriler alınır.

Alt sorgu sadece var olup olmadığını kontrol eder.

Alternatif olarak IN ile:
...
✅ Soru 4: 1 numaralı mağazada bulunmayan filmler
🧠 Beklenen: NOT EXISTS veya NOT IN kullanımı
...
inventory tablosunda store_id = 1 olan eşleşmeler aranır.

Eğer hiç eşleşme yoksa, o film bu mağazada yoktur → alınır.

✅ Soru 5: Her kategoride en uzun film
🧠 Beklenen: correlated subquery veya ROW_NUMBER() kullanımına açık

Yöntem 1: correlated subquery
sql
Kopyala
Düzenle
...
Her category_id için, o kategorideki en uzun filmi buluruz.

WHERE f.length = MAX(...) ile çalışır.
...
ROW_NUMBER() ile her kategori için en uzun film sıralanır.

rn = 1 olanlar seçilerek sonuç alınır.
...
✅ 4. Birden fazla satır döndüren subquery'ler, yalnızca çok-değerli operatörlerle kullanılabilir:
...
❌ 5. Subquery doğrudan BETWEEN içinde kullanılamaz
